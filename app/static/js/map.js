	...
	fetch(`/api/isochrones?${params}`)
		...
		.then((data) => {
			...
			if (data.features) {
				...
				processFeatures.forEach((feature, index) => {
					...
					if (isCustomRequest) {
						const originalIndex = isCustomRequest
							? processFeatures.length - 1 - index
							: index;
					...
					const layer = L.geoJSON(feature, {
						...
						.bindTooltip(
							`<div class="time-tooltip"><span class="neon-text">${minutes} mins</span></div>`,
							{
								permanent: false,
								direction: "center",
		...
		.catch((error) => {
			...
			if (
				...
				console.log(
					"Rate limit error detected - showing error message"
				);
				showRateLimitError();
			} else {
...
`;
	...
	item.addEventListener("click", (e) => {
		if (!e.target.closest("button")) {
			...
			setTimeout(() => {
				const travelTimeBtn =
					document.querySelector(".travel-time-btn");
				if (travelTimeBtn) {
					travelTimeBtn.addEventListener("click", function () {
						const btnLat = parseFloat(
							this.getAttribute("data-lat")
						);
						const btnLng = parseFloat(
...
function handleViewSelectedButton() {
	...
	if (checkedBoxes.length > 0) {
		if (!viewSelectedButton) {
			...
			viewSelectedButton.innerHTML =
				'<i class="bi bi-eye-fill"></i> View Selected POIs';
...
function captureSelectedArea() {
	...
	setTimeout(() => {
		...
		const mapControls = document.querySelector(
			...
			.then(function (canvas) {
				...
				fetch("/save-screenshot", {
					...
					.then((data) => {
						if (data.success) {
							showToast(
								"Screenshot saved on server: " + data.filename
							);
						} else {
							showToast(
								"Failed to save screenshot: " + data.error
							);
						}
					})
					.catch((error) => {
	...
	poiItems.forEach((item) => {
		...
		if (nameElement && categoryElement && marker) {
			...
			if (!matchesTime && actualTravelTime > 0) {
				switch (time) {
					case "15":
						...
						console.log(
							`${name}: ${actualTravelTime} <= 15 = ${matchesTime}`
						);
						break;
					case "30":
						matchesTime =
							actualTravelTime > 15 && actualTravelTime <= 30;
						console.log(
							`${name}: 15 < ${actualTravelTime} <= 30 = ${matchesTime}`
						);
						break;
					case "45":
						matchesTime =
							actualTravelTime > 30 && actualTravelTime <= 45;
						console.log(
							`${name}: 30 < ${actualTravelTime} <= 45 = ${matchesTime}`
						);
						break;
					case "60":
						matchesTime =
							actualTravelTime > 45 && actualTravelTime <= 60;
						console.log(
							`${name}: 45 < ${actualTravelTime} <= 60 = ${matchesTime}`
						);
						break;
				}
			}
			...
			if (shouldShow) {
				item.style.display = "";
				const isFiltered =
					searchTerm !== "" || category !== "all" || time !== "all";
				item.style.backgroundColor = isFiltered
					? "rgba(0, 255, 163, 0.1)"
					: "";
...
function addToSidebar(poi) {
	...
	item.querySelector(".btn-edit").addEventListener("click", () => {
		...
		document.getElementById("poi-category").value =
		...
		const editMarker = L.marker([poi.latitude, poi.longitude])
			.addTo(map)
			.bindPopup("Editing location")
			.openPopup();
...
// Added js formatinh using prettier
